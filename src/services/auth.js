...
export const AuthService = {
    ...
    async register(userData) {
        ...
        if (!condoSnap.exists()) throw new Error("Código do condomínio inválido!");
        
        const cred = await createUserWithEmailAndPassword(auth, userData.email, userData.password);
        await setDoc(doc(db, "users", cred.user.uid), {
            name: userData.name, apartment: userData.apartment, email: userData.email, type: 'resident', condoId: userData.condoId
        });
        return cred.user;
    },
    ...
    async createSyndicUser(data, condoId) {
        ...
        await setDoc(doc(db, "users", cred.user.uid), {
            name: data.name, email: data.email, type: 'syndic', condoId: condoId
        });
        return cred.user;
    },
