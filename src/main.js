...
createApp({
    setup() {
                ...
                try {
                    ...
                    if (profile) {
                        ...
                        if (profile.type === 'superuser') {
                            ...
                            DataService.listenCondos((data) => store.condoList = data);
                        } else {
        ...
        const confirmBlock = async () => {
            try {
                await DataService.addReservation({
                    condoId: store.currentUser.condoId, spaceId: selectedSpace.value.id, userId: store.currentUser.uid,
                    userName: "BLOQUEIO", date: form.value.date, time: "DIA TODO", status: 'maintenance', reason: form.value.reason
                });
