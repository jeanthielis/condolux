import { createApp, onMounted, computed, ref } from 'https://unpkg.com/vue@3/dist/vue.esm-browser.js';
...
createApp({
    setup() {
        ...
        // Listas Convidados/Fornecedores
        const guestList = ref([]);
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                try {
                    ...
                    if (profile && profile.blocked) { await AuthService.logout(); Swal.fire('Bloqueado', 'Contate o síndico.', 'error'); return; }

                    if (profile) {
                        store.currentUser = { uid: user.uid, email: user.email, ...profile };
                        if (profile.type === 'superuser') {
                            store.currentView = 'admin';
                            DataService.listenCondos((d) => store.condoList = d);
        ...
        // --- GESTÃO DE LISTAS (CONVIDADOS) ---
        const addGuest = () => {
            if(!tempGuest.value.name) return;
            guestList.value.push({ ...tempGuest.value, checkedIn: false });
            tempGuest.value = { name: '', doc: '' };
        };
        const removeGuest = (i) => guestList.value.splice(i, 1);

        const addSupplier = () => {
            if(!tempSupplier.value.name) return;
            supplierList.value.push({ ...tempSupplier.value, checkedIn: false });
            tempSupplier.value = { name: '', company: '' };
        };
        ...
        const toggleCheckin = async (person, type) => {
            person.checkedIn = !person.checkedIn; // Atualiza visual
            try {
                // Atualiza no banco
                ...
                person.checkedIn = !person.checkedIn; // Reverte erro
        ...
        const closeAllModals=()=>{showModal.value=false;adminCtrl.showCondoModal.value=false;showBlockModal.value=false;showDateDetails.value=false;showStaffModal.value=false;showCheckinModal.value=false;};
        ...
        const saveSpace=async()=>{if(!adminCtrl.newSpace.value.timeSlotsString){if(tempTimeSlots.value.length>0)adminCtrl.newSpace.value.timeSlots=[...tempTimeSlots.value];else adminCtrl.newSpace.value.timeSlots=['08:00 - 12:00','13:00 - 17:00','18:00 - 22:00'];}else{adminCtrl.newSpace.value.timeSlots=adminCtrl.newSpace.value.timeSlotsString.split(',').map(s=>s.trim());}await adminCtrl.addSpace();tempTimeSlots.value=[];};
        ...
        const confirmBooking=async()=>{
            try{
                await DataService.addReservation({
                    ...
                    guests: guestList.value, suppliers: supplierList.value
                });
        ...
        // --- COMPUTEDS (DEFINIÇÕES QUE FALTAVAM) ---
        ...
        // Aqui estão as definições de myReservations, etc.
